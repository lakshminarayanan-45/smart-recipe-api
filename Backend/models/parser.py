...
def parse_ingredient_line(text):
    """
    Parse raw ingredient text into structured data with amount, unit & name.
    Handles quantities appearing anywhere in the line, including after dashes.
    Skips injection for unquantifiable ingredients (e.g., salt to taste).
    """
    items = [i.strip() for i in re.split(r",|\n", text) if i.strip()]
    results = []
    for item in items:
        item = item.replace("–", "-")
        # Regex captures qty/unit possibly near start or just before ingredient name
        pattern = r"^(?P<qty>[\d\s\/\.]+)?\s*(?P<unit>[^\d\s\-]+)?\s*[\-–:]?\s*(?P<name>.+)$"
        ...
        # If qty missing, try finding last number in line (e.g., 'ingredient - 2 cups')
        if not qty or qty.strip() == "":
            nums = re.findall(r"[\d\.]+", item)
            qty = nums[-1] if nums else None

            if qty:
                # Remove number and separator from name
                name = re.sub(r"[\-–:]?\s*" + re.escape(qty), "", item).strip()

        try:
            amount = eval(qty) if qty else None
            if amount is None:
                amount = 0  # Use 0 to flag unquantifiable lines
        except Exception:
            amount = 0

        # Skip quantity for salt to taste, as needed, etc.
        if any(x in name.lower() for x in ['salt to taste', 'as needed', 'to taste']):
            amount = 0
...
def scale_cooking_time(original_time, new_servings, base=2):
    """
    Logarithmic capped time scaling for better realism.
    """
    try:
